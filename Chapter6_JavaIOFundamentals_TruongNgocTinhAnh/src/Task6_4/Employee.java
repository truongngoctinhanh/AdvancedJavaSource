...
public class Employee {
	...
	public double calculateIncome() {
		...
		if (this.getCoefficient() < 0)
			throw new ArithmeticException("Coefficient is not valid!");
		if (this.getAllowance() < 0)
			throw new ArithmeticException("Allowance is not valid!");
	...
	public double calculateTaxableIncome() {
		...
		if (this.getAmountOfPeople() < 0)
			throw new ArithmeticException(
					"Amount of people depend is not valid!");
		salaryTax = this.calculateIncome() - AMOUNT1 - this.getAmountOfPeople()
				* AMOUNT2;
		return salaryTax;
	}
	...
	public double calculatePersonalIncomeTax() {
		...
		if (calcTaxableIncome < PersonalIncomeTax.RATE1.getTaxableIncome()) {
			result = PersonalIncomeTax.RATE1
					.calculatePersonalIncomeTax(calcTaxableIncome);
		} else if (calcTaxableIncome < PersonalIncomeTax.RATE2
				.getTaxableIncome()) {
			result = PersonalIncomeTax.RATE2
					.calculatePersonalIncomeTax(calcTaxableIncome);
		} else if (calcTaxableIncome < PersonalIncomeTax.RATE3
				.getTaxableIncome()) {
			result = PersonalIncomeTax.RATE3
					.calculatePersonalIncomeTax(calcTaxableIncome);
		} else if (calcTaxableIncome < PersonalIncomeTax.RATE4
				.getTaxableIncome()) {
			result = PersonalIncomeTax.RATE4
					.calculatePersonalIncomeTax(calcTaxableIncome);
		} else if (calcTaxableIncome < PersonalIncomeTax.RATE5
				.getTaxableIncome()) {
			result = PersonalIncomeTax.RATE5
					.calculatePersonalIncomeTax(calcTaxableIncome);
		} else if (calcTaxableIncome < PersonalIncomeTax.RATE6
				.getTaxableIncome()) {
			result = PersonalIncomeTax.RATE6
					.calculatePersonalIncomeTax(calcTaxableIncome);
		} else {
			result = PersonalIncomeTax.RATE7
					.calculatePersonalIncomeTax(calcTaxableIncome);
		}
		return result;
	}
	...
	public void DisplayInfoEmployee() {
		...
		System.out
				.println("Amount of dependent people: " + this.amountOfPeople);
		System.out.println("Allowance: " + Double.parseDouble(df.format(this.allowance)));
		System.out.println("Actual Income: " + Double.parseDouble(df.format(calculateActualIncome())));
	}
}
